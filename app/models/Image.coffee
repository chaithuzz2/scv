  ...
  watershed:()=>
    ...
    step2 = (i,j) ->
      if distance[i][j] isnt 1
        ...
        for k in [-1,0,1]
          for l in [-1,0,1]
            if greyinf[i+k][j+l] is greyinf[i][j] and distance[i+k][j+l] > 0 and distance[i+k][j+l] < min
              min = distance[i+k][j+l]
        if min isnt VMAX and distance[i][j] isnt min+1
          distance[i][j] = min+1
    console.log "step2 process"      
    while scan_step2 is 1
      for i in [0..@height-1]
        for j in [0..@width-1]
          comp[i][j] = distance[i][j]
      #console.log "comp copied"    
      for i in [1..@height-2]
        for j in [1..@width-2]
          step2(i,j)
    ...
    step3 = (i,j) ->
      ...
      if distance[i][j] is 0
        for k in [-1,0,1]
          for l in [-1,0,1]
            if greyinf[i+k][j+l] is greyinf[i][j] and labels[i+k][j+l] > 0 and labels[i+k][j+l] < lmin
              lmin = labels[i+k][j+l]
      ...
      if distance[i][j] is 1
        ...
        for k in [-1,0,1]
          for l in [-1,0,1]
            if greyinf[i+k][j+l] is greyinf[i][j] and distance[i+k][j+l]  is distance[i][j]-1 and labels[i+k][j+l]>0 and labels[i][j] < lmin
              lmin = labels[i+k][j+l]
      if lmin isnt LMAX and labels[i][j] isnt lmin 
        labels[i][j] = lmin
    console.log "step3 process"    
    ...
    console.log "step3 completed successfully"
    max = labels[0][0]
    for i in [0..@height-1]
      for j in [0..@width-1]
        if labels[i][j] > max
    ...
    for i in [0..@height-1]
      for j in [0..@width-1]
        ...
        #out.data[a+1] = (labels[i][j]/max) * 255
        #out.data[a+2] = (labels[i][j]/max) * 255
        out.data[a+3] = (labels[i][j]/max) * 255
        #else 
         # out.data[a] = 255
          #out.data[a+1] = 255
          #out.data[a+2] = 255
